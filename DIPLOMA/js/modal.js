export default class Modal {
  ...
  onDocumentClick(event) {
    const targetLink = event.target.closest("[data-modal-link]");

    if (targetLink) {
      const target = targetLink.dataset.modalLink;
      const speed = targetLink.dataset.modalSpeed;
  ...
  open() {
		document
			.querySelectorAll(".modal__window")
			.forEach((el) => el.classList.remove("modal__window--open"));
